...
// Cada item aqui é um IT/APR/MANUAL da Biblioteca (não uma prova pré-cadastrada) —
// o Simulado gera as 10 perguntas na hora, via IA, direto do PDF, então todo
// IT/APR/MANUAL do Contrato aparece aqui, para qualquer Função.
type SimuladoExam = {
  id: number;
  title: string;
...
// (oficial).
//
// `operatorOnly` (usado por /simulado/operadores) restringe a lista de
// Função só às marcadas como "Operador" em Funções (roles.isOperator) — o
// resto do fluxo (Contrato, IT/APR/MANUAL, categoria, tempo de prova) é
// idêntico ao Simulado padrão.
export default function SimuladoForm({ operatorOnly = false }: { operatorOnly?: boolean }) {
  const [step, setStep] = useState<Step>({ kind: "form" });

  const [sectors, setSectors] = useState<Sector[]>([]);
  const [roles, setRoles] = useState<Role[]>([]);
  const [exams, setExams] = useState<SimuladoExam[]>([]);
  // Filtro por categoria (ex.: "Guindastes", "Empilhadeiras") — útil sobretudo
  // pra Contratos como Treinamentos, que podem ter muitos IT/APR/MANUAL.
  ...
  const categories = Array.from(new Set(exams.map((e) => e.category).filter((c): c is string => !!c))).sort();
  const filteredExams = categoryFilter ? exams.filter((e) => e.category === categoryFilter) : exams;
  const visibleRoles = operatorOnly ? roles.filter((r) => r.isOperator) : roles;
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
              <h1 className="text-lg font-semibold text-slate-900">
                {operatorOnly ? "Simulado de Operadores" : "Fazer um Simulado"}
              </h1>
              ...
              <p className="mt-2 text-xs text-slate-400">
                ...
                {operatorOnly ? (
                  <>
                    Não é operador?{" "}
                    <Link href="/simulado" className="underline hover:text-slate-600">
                      Simulado padrão
                    </Link>
                  </>
                ) : (
                  <>
                    É operador (guindaste, empilhadeira...)?{" "}
                    <Link href="/simulado/operadores" className="underline hover:text-slate-600">
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  {operatorOnly && !loadingRoles && visibleRoles.length === 0 && (
                    <p className="mt-1 text-xs text-amber-700">
                      Nenhuma função marcada como Operador foi cadastrada ainda. Peça pro admin
                      marcar em Funções.
                    </p>
                  )}
                </div>
            ...
            {sectorId && roleId && (
              <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 className="text-sm font-semibold text-slate-900">Escolha o IT, APR ou MANUAL do simulado</h2>
                <p className="mt-1 text-xs text-slate-500">Disponíveis para esse Contrato:</p>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {!loadingExams && exams.length === 0 && (
                    <p className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-400">
                      Nenhum IT/APR/MANUAL cadastrado na Biblioteca para esse Contrato ainda.
                    </p>
                  )}
                  {!loadingExams && exams.length > 0 && filteredExams.length === 0 && (
                    <p className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-400">
                      Nenhum item para a categoria &quot;{categoryFilter}&quot;.
                    </p>
                  )}
                    ...
                    >
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900">
                          {ex.title}{" "}
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${DOCUMENT_TYPE_BADGE[ex.documentType]}`}
                          >
                            {ex.documentType}
                          </span>{" "}
                          {ex.category && (
