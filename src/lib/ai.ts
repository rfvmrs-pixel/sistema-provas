...
export async function generateExamFromText(
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 8000,
    system:
      `Você é um especialista em treinamento corporativo e elaboração de avaliações (provas) de múltipla escolha em português do Brasil, a partir de material de treinamento (manuais, procedimentos, políticas internas de empresas de logística). ${DOCUMENT_TYPE_GUIDANCE[documentType]} Gere questões claras, objetivas, que testem compreensão real do conteúdo (não só decoreba de frases soltas), com exatamente 4 alternativas cada, apenas uma correta. ALTERNATIVAS ERRADAS (as 3 incorretas): devem ser CLARAMENTE erradas para quem conhece o procedimento — cada uma com um erro marcante e fácil de apontar (ex.: etapa fora de ordem, EPI ou medida de controle errada, responsável errado, valor/limite muito diferente, atitude insegura ou proibida pelo documento). NÃO escreva alternativas erradas que sejam quase iguais à correta, paráfrases dela, ou que só troquem uma palavra/detalhe sutil — o objetivo é avaliar se o colaborador sabe o procedimento, não pegá-lo em pegadinha de redação. As 3 erradas também devem ser diferentes entre si (cada uma com um erro de natureza diferente), soar verossímeis como ação de trabalho (nada absurdo ou engraçado) e ter tamanho parecido com a correta, para que a certa não se destaque pelo tamanho. Não use "todas as anteriores" nem "nenhuma das anteriores". Sempre classifique cada questão com um 'topic' curto (2-5 palavras) que identifique o tema/assunto dentro do documento, para permitir análise posterior de quais temas os funcionários têm mais dificuldade.${
